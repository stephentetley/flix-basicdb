...
public class MapRow {
    ...
    protected MapRow(Map<String,Object> map1) {
        row = map1;
    }
    ...
    public int getInt(String label) throws Exception {
        if (row.get(label).getClass() == Integer.class) {
    ...
    public String getString(String label) throws Exception {
        if (row.get(label).getClass() == String.class) {
    ...
    public BigDecimal getBigDecimal(String label) throws Exception {
        if (row.get(label).getClass() == BigDecimal.class) {
            return (BigDecimal) row.get(label);
        } else {
            throw new SQLException();
        }
    }
